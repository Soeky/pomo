...
func TestSetValueAndGetValueForExtendedKeys(t *testing.T) {
	...
	if err := SetValue("break_credit_threshold_minutes", "15"); err != nil {
		t.Fatalf("SetValue break_credit_threshold_minutes failed: %v", err)
	}
	...
	gotBreakThreshold, err := GetValue("break_credit_threshold_minutes")
	if err != nil {
		t.Fatalf("GetValue break_credit_threshold_minutes failed: %v", err)
	}
	if gotBreakThreshold != "15" {
		t.Fatalf("unexpected break_credit_threshold_minutes: %s", gotBreakThreshold)
	}
}
...
func TestSetValueValidationForExtendedKeys(t *testing.T) {
	...
	if err := SetValue("break_credit_threshold_minutes", "0"); err == nil {
		t.Fatalf("expected break_credit_threshold_minutes validation error")
	}
}
