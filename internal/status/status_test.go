...
func TestCurrentStatus_ActiveFocusAndOverdue(t *testing.T) {
	...
	if _, err := db.InsertSessionAt("focus", "Deep Work", start, 20*time.Minute); err != nil {
		t.Fatalf("insert tracked focus event: %v", err)
	}
...
func TestCurrentStatus_BreakTypeUsesSleepEmoji(t *testing.T) {
	...
	start := time.Date(2026, 2, 25, 10, 0, 0, 0, time.UTC)
	if _, err := db.InsertSessionAt("break", "", start, 5*time.Minute); err != nil {
		t.Fatalf("insert tracked break event: %v", err)
	}
...
func TestCurrentStatus_OverdueOddSecondUsesTomatoEmoji(t *testing.T) {
	...
	start := time.Date(2026, 2, 25, 10, 0, 0, 0, time.UTC)
	if _, err := db.InsertSessionAt("focus", "X", start, time.Minute); err != nil {
		t.Fatalf("insert tracked focus event: %v", err)
	}
